...
** TODO Improve the python-flask-cicd-code repository so that it can output a docker image
** Test, debug and make sure it works when run manually
** Manually upload the docker image either to AWS or Dockerhub
** Describe the process in a declarative Jenkinsfile, automating the process of docker image creation in a manually setup Jenkins
