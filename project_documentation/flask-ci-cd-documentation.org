...
**** Had some issues with adding the hashicorp repo with ansible, but found out the reason trough comparisition with regular bash instructions
...
via bitwarden-cli.
**** TODO Manually upload the image from docker, following the tutorial
...
** TODO Making the project more presentable (on GitHub) - .org html export
Plan is to add a git hook that will export project docs from .org to
.html and then stage the new .html file in the commit. This file can
than be linked to from the project's README.

Looks like I haven't created a project's README yet.
*** TODO Create a project's README
